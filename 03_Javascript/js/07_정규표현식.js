...
document.getElementById("check1").addEventListener("click", function(){
    ...
    // "Javat"와 일치하는 문자열이 있는지 검사하는 정규식 객체
    const regEx2 = /java/;
...
document.getElementById("check2").addEventListener("click", function(){
    ...
    const regEx3 = /^[abcd]/;
    div1.innerHTML += "/^[abcd]/, group : " + regEx2.test("group") + "<hr>";
    div1.innerHTML += "/^[abcd]/, car : " + regEx2.test("car") + "<hr>";
    div1.innerHTML += "/^[abcd]/, dark : " + regEx2.test("dark") + "<hr>";

    // $(달러) : 문자열의 끝을 의미
    const regEx4 = /team$/;
    div1.innerHTML += "/teag$/, A-team : " + regEx4.test("A-team") + "<hr>";
    div1.innerHTML += "/teag$/, team-A : " + regEx4.test("team-A") + "<hr>";

    // ^[ABCD]-team$
    // A-team, B-team, C-team, D-team이 맞으면 true 아니면 false
...
// 이름 유효성 검사
// 문서 내에 id가 inputName인 요소를 가져와서,
// 그 요소에 keyup 이벤트가 발생할 때마다 함수를 수행하는 EventListener를 추가
